...
default_branch=$(
  gh_api "/repos/$OWNER/$REPO" --jq '.default_branch'
)
[ -n "$default_branch" ] || die "could not detect default branch for $OWNER/$REPO"
...
apply_ruleset() {
  ...
  if [ "$ruleset_name" = "null" ] || [ -z "$ruleset_name" ]; then
    die "ruleset name missing in $ruleset_file"
  fi
...
desired_ruleset_names=$(
  for ruleset_file in "$RULESET_DIR"/*.json; do
    jq -r '.name' "$ruleset_file"
  done | jq -R . | jq -s .
)
existing_rulesets=$(json_tmp)
gh_api "/repos/$OWNER/$REPO/rulesets?includes_parents=false&per_page=100" >"$existing_rulesets"
while IFS= read -r stale_ruleset; do
  stale_ruleset_id=$(printf '%s\n' "$stale_ruleset" | jq -r '.id')
  stale_ruleset_name=$(printf '%s\n' "$stale_ruleset" | jq -r '.name')
  gh_api --method DELETE "/repos/$OWNER/$REPO/rulesets/$stale_ruleset_id" >/dev/null
  printf 'Removed stale repository branch ruleset: %s\n' "$stale_ruleset_name"
done < <(
  jq -c --argjson desired "$desired_ruleset_names" '
    .[]
    | select(.source_type == "Repository")
    | select(.target == "branch")
    | select((.name as $name | $desired | index($name)) | not)
    | {id, name}
  ' "$existing_rulesets"
)
rm -f "$existing_rulesets"

# Standards are enforced by repository rulesets, not classic branch protection.
# Remove any legacy default-branch protection after rulesets are active so old
# approval requirements do not reintroduce a human gate for solo-developer repos.
if gh_api "/repos/$OWNER/$REPO/branches/$encoded_default_branch/protection" >/dev/null 2>&1; then
  gh_api --method DELETE "/repos/$OWNER/$REPO/branches/$encoded_default_branch/protection" >/dev/null
  printf 'Removed legacy classic branch protection from %s\n' "$default_branch"
else
  printf 'No legacy classic branch protection found on %s\n' "$default_branch"
fi
